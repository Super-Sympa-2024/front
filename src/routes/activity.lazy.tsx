...
function RouteComponent() {
  ...
  return (
    <div className='flex w-full h-screen overflow-hidden'>
      <Link className='absolute right-0 top-0 m-10 border-[3px] p-1 border-green-500' to="/">Accueil</Link>
      <div className='flex flex-col w-2/3 relative'>
        <p className='text-center text-2xl mt-5 font-bold mx-20'>
          Résolvez les problèmes qui se trouvent dans l'océan en cliquant sur les objets qui
          les représentent pour retablir la santer de l'humain et celle de l'océan :)
        </p>
        {index >= 0 && (
          <div className='flex justify-center gap-4 mt-10 p-10 absolute top-10'>
            <div>
              <h2 className='font-bold'>Relation entre l'océan et l'humain : </h2>
              <p>{relations[index]}</p>
            </div>
            <div>
        ...
        {index === relations.length -1 && (
          <p className='text-center text-green-500 text-xl py-3'> Bien joué ! Vous en savez maintenant plus sur les océans et ses relations avec l'humain </p>
        )}
