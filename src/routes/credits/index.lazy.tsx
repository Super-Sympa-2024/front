...
export default function AbyssesRouteComponent() {
  const [organizationMembersData, setOrganizationMembersData] = useState<
    listOrganizationMembersResponse['data']
  >([])
  ...
  const { getOrganizationMembers, getNumberOfCommitsByUser } =
    useGithubService()
  ...
  useEffect(() => {
    const fetchNumberOfCommitsByUser = async () => {
      const results = await Promise.all(
        organizationMembersData.map(async (organizationMember) => {
          try {
            const data = await getNumberOfCommitsByUser(
      ...
      const filteredResults = results.filter(
        (result): result is { number: number; login: string } => result !== null
      )
      setNumberOfCommitsByUser(
        filteredResults.length > 0 ? filteredResults : undefined
      )
    }
        ...
        {zones.map((zone, index) => {
          ...
          return (
            ...
            >
              ...
              <h1 className='text-4xl font-bold mb-4 z-10'>{zone.name}</h1>
              <p className='text-xl mb-6 text-center px-4 z-10'>
                {zone.description}
              </p>
        ...
        >
          ...
          <div className='flex flex-wrap justify-center items-center relative'>
            {organizationMembersData.length > 0 ? (
              organizationMembersData.map((organizationMembers) => {
                const commits = numberOfCommitsByUser?.find(
                  (user) => user.login === organizationMembers.login
                )
                return (
                  <div
                    key={organizationMembers.id}
                    className='flex flex-col items-center mt-48 m-12'
                  >
                    <img
                      className='size-24 rounded-full mb-2 float'
                      src={organizationMembers.avatar_url}
                      alt={`Avatar de ${organizationMembers.login}`}
                    />
                    <a
                      href={organizationMembers.html_url}
                      target='_blank'
                    ...
                    <p className='float'>
                      Commits: {commits ? commits.number : 0}
                    </p>
                  </div>
                )
              })
          ...
          <div className='relative h-full w-full bg-transparent flex items-end justify-evenly'>
            <img src={algue1} alt='Algue 1' className='w-24 h-auto mb-0' />
            <img
              src={alguebronzee}
              alt='Algue 2'
              className='w-32 h-auto mb-0'
            />
            <img src={algue3} alt='Algue 3' className='w-28 h-auto mb-0' />
            <img src={algue2} alt='Algue 3' className='w-28 h-auto mb-0' />
            <img
              src={alguebronzee}
              alt='Algue 3'
